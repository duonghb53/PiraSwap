...
import {IPiraPair} from "./interfaces/IPiraPair.sol";
import {IPiraRouter02} from "./interfaces/IPiraRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by PiraSwap teams.
 */
contract PiraZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // PiraRouter interface
    IPiraRouter02 public PiraRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from PiraSwap)
    ...
    // Address PiraRouter
    address private PiraRouterAddress;
     ...
     * @param _PiraRouter: address of the PiraRouter
    ...
    constructor(
        ...
        PiraRouterAddress = _PiraRouter;
        PiraRouter = IPiraRouter02(_PiraRouter);
    ...
    {
        address token0 = IPiraPair(_lpToken).token0();
        address token1 = IPiraPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IPiraPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = PiraRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = PiraRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IPiraPair(_lpToken).token0() || _token0ToZap == IPiraPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IPiraPair(_lpToken).token0() || _token1ToZap == IPiraPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IPiraPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IPiraPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IPiraPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = PiraRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = PiraRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IPiraPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "PiraRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "PiraRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        PiraRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), PiraRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(PiraRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = PiraRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = PiraRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = PiraRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = PiraRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = PiraRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
